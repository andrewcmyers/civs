...
sub init {
    ...
    $name = $data->{'name'};                 # name of supervisor: UTF-8
    $title = $data->{'title'};               # title of poll: HTML
    $email_addr = $data->{'email_addr'};     # email of supervisor: UTF-8
    $description = $data->{'description'};   # poll description: HTML
    $addresses = $data->{'addresses'};
    @addresses = split /[\r\n]+/, $addresses;
    $election_begin = $data->{'election_begin'};
    $election_end = $data->{'election_end'};  # HTML
    $public = $data->{'public'};
